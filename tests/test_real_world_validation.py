...
class RealWorldValidator:
    ...
    def __init__(self):
        ...
        # Test data
        self.test_queries = {
            "arxiv": ["machine learning education", "mathematics pedagogy", "computer science curriculum"],
            "wikipedia": ["Mathematics", "Educational technology", "Pedagogy"],
            "dictionary": ["education", "pedagogy", "curriculum"],
            "openlibrary": ["mathematics textbook", "science education", "teaching methods"],
        }
    ...
    async def initialize_services(self):
        ...
        # Initialize tools
        self.arxiv_tool = ArxivTool(self.config, self.cache_service, self.rate_limiting_service, self.usage_service)
        self.wikipedia_tool = WikipediaTool(self.config, self.cache_service, self.rate_limiting_service, self.usage_service)
        self.dictionary_tool = DictionaryTool(self.config, self.cache_service, self.rate_limiting_service, self.usage_service)
        self.openlibrary_tool = OpenLibraryTool(self.config, self.cache_service, self.rate_limiting_service, self.usage_service)
    ...
    async def cleanup_services(self):
        ...
        clients = [self.arxiv_client, self.wikipedia_client, self.dictionary_client, self.openlibrary_client]
    ...
    async def run_test(self, test_name: str, api_service: str, test_func) -> TestResult:
        ...
        try:
            ...
            result = TestResult(test_name=test_name, api_service=api_service, status="PASS", duration_seconds=duration, details=details)
            print(f"    ✅ {test_name} passed ({duration:.2f}s)")

        except Exception as e:
            duration = time.time() - start_time
            result = TestResult(test_name=test_name, api_service=api_service, status="FAIL", duration_seconds=duration, error_message=str(e))
    ...
    async def test_arxiv_basic_search(self) -> Dict[str, Any]:
        ...
        papers = await self.arxiv_client.search_papers(self.test_queries["arxiv"][0], max_results=5)
    ...
    async def test_arxiv_educational_features(self) -> Dict[str, Any]:
        ...
        """
        papers = await self.arxiv_tool.search_academic_papers(query="mathematics education", subject="Mathematics", academic_level="Undergraduate", max_results=3)
        if len(papers) == 0:
    ...
    async def test_arxiv_research_trends(self) -> Dict[str, Any]:
        ...
        """
        trends = await self.arxiv_tool.analyze_research_trends(subject="Computer Science", days=14)
        ...
        return {"total_papers_analyzed": trends.get("total_papers", 0), "insights_generated": len(trends.get("educational_insights", []))}
    ...
    async def test_wikipedia_search(self) -> Dict[str, Any]:
        ...
        results = await self.wikipedia_client.search_wikipedia(self.test_queries["wikipedia"][0], limit=5)
    ...
    async def test_wikipedia_educational_features(self) -> Dict[str, Any]:
        ...
        """
        articles = await self.wikipedia_tool.search_educational_articles(query="mathematics education", subject="Mathematics", grade_level="9-12", limit=3)
        if len(articles) == 0:
    ...
    async def test_dictionary_word_definition(self) -> Dict[str, Any]:
        ...
        definition = await self.dictionary_client.get_definition(self.test_queries["dictionary"][0])
        assert definition is not None, "Failed to get word definition"
        assert "meanings" in definition, "Missing meanings in definition"
        return {"word": definition.get("word", ""), "meanings_count": len(definition.get("meanings", []))}
    ...
    async def test_dictionary_phonetics(self) -> Dict[str, Any]:
        ...
        return {"has_phonetics": bool(phonetics), "phonetic_text": phonetics.get("text", "") if phonetics else ""}
    ...
    async def test_dictionary_comprehensive_data(self) -> Dict[str, Any]:
        ...
        return {"word": data.get("word", ""), "has_meanings": "meanings" in data, "has_phonetics": "phonetics" in data}
    ...
    async def test_dictionary_educational_features(self) -> Dict[str, Any]:
        ...
        return {"educational_relevance": educational_relevance, "complexity_level": complexity, "grade_levels": len(grade_levels_data)}
    ...
    async def test_openlibrary_book_search(self) -> Dict[str, Any]:
        ...
        books = await self.openlibrary_client.search_books(self.test_queries["openlibrary"][0], limit=5)
    ...
    async def test_openlibrary_book_details(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if details:
                return {"isbn": test_isbn, "title": details.get("title", "")[:50], "has_authors": "authors" in details}
            else:
                ...
                if books and "isbn" in books[0]:
                    ...
                    return {"isbn": isbn, "title": details.get("title", "")[:50] if details else "Not found", "has_authors": "authors" in details if details else False}
    ...
    async def test_openlibrary_educational_features(self) -> Dict[str, Any]:
        ...
        """
        books = await self.openlibrary_tool.search_educational_books(query="mathematics textbook", subject="Mathematics", grade_level="9-12", limit=3)
        if len(books) == 0:
    ...
    async def test_openlibrary_book_recommendations(self) -> Dict[str, Any]:
        ...
        """
        recommendations = await self.openlibrary_tool.get_book_recommendations(subject="Science", grade_level="6-8", limit=3)
        assert len(recommendations) > 0, "No book recommendations found"

        return {"recommendations_count": len(recommendations), "subjects_covered": len(set(book.get("subject", ["Unknown"])[0] for book in recommendations))}
    ...
    async def test_cross_api_educational_workflow(self) -> Dict[str, Any]:
        ...
        papers = await self.arxiv_tool.search_academic_papers(query=topic, subject="Mathematics", max_results=2)

        # 2. Get Wikipedia articles on the topic
        articles = await self.wikipedia_tool.search_educational_articles(query=topic, subject="Mathematics", limit=2)

        # 3. Get definitions for key terms
        definitions = await self.dictionary_tool.get_word_definition(word="mathematics", grade_level="9-12")

        # 4. Find relevant books
        books = await self.openlibrary_tool.search_educational_books(
            query=topic,
            subject="Mathematics",
            limit=2,  # Corrected to limit
        )

        return {"papers_found": len(papers), "articles_found": len(articles), "definitions_retrieved": bool(definitions), "books_found": len(books), "workflow_complete": True}
    ...
    async def test_performance_under_load(self) -> Dict[str, Any]:
        ...
        return {"concurrent_requests": len(tasks), "successful_requests": successful_requests, "total_duration": duration, "average_response_time": duration / len(tasks)}
    ...
    def generate_report(self) -> ValidationReport:
        ...
        performance_metrics = {
            ...
            "api_response_times": {api: [r.duration_seconds for r in self.test_results if r.api_service == api] for api in ["arxiv", "wikipedia", "dictionary", "openlibrary"]},
        }

        # Educational features validation
        educational_features = {
            "educational_metadata_present": len([r for r in self.test_results if r.status == "PASS" and "Educational Features" in r.test_name]),
            "cross_api_workflow_success": any(r.status == "PASS" and "Cross-API" in r.test_name for r in self.test_results),

...
class PerformanceTester:
    ...
    async def measure_operation(self, operation_name: str, operation_func, *args, **kwargs) -> Tuple[Any, float]:
        ...
        try:
            ...
            if operation_name not in self.metrics:
                self.metrics[operation_name] = PerformanceMetrics(operation=operation_name, response_times=[], success_count=0, error_count=0, cache_hits=0, cache_misses=0)
    ...
    async def run_concurrent_operations(self, operation_name: str, operation_func, args_list: List[Tuple], max_workers: int = 10) -> List[Tuple[Any, float]]:
    ...
    def get_performance_report(self) -> Dict[str, Any]:
        ...
        if total_requests > 0:
            report["summary"]["overall_success_rate"] = report["summary"]["total_successes"] / total_requests
...
class TestResponseTimePerformance:
    ...
    async def test_book_search_response_times(self, performance_tester):
        ...
        with patch("src.api.openlibrary.OpenLibraryAPI.search_books") as mock_search:
            mock_search.return_value = [{"title": "Test Book", "author": "Test Author", "grade_level": "6-8", "educational_value": 0.9}]
            ...
            for query, subject, grade_level in search_queries:
                result, response_time = await performance_tester.measure_operation("search_educational_books", search_educational_books, ctx, query=query, subject=subject, grade_level=grade_level)
    ...
    async def test_article_search_response_times(self, performance_tester):
        ...
        with patch("src.api.wikipedia.WikipediaAPI.search_articles") as mock_search:
            mock_search.return_value = [{"title": "Test Article", "summary": "Test summary", "grade_level": "6-8", "educational_value": 0.85}]
            ...
            for query, subject, grade_level in search_queries:
                result, response_time = await performance_tester.measure_operation(
                    "search_educational_articles", search_educational_articles, ctx, query=query, subject=subject, grade_level=grade_level
                )
    ...
    async def test_definition_response_times(self, performance_tester):
        ...
        with patch("src.api.dictionary.DictionaryAPI.get_definition") as mock_definition:
            mock_definition.return_value = {"word": "test", "definition": "A test definition", "grade_level": "6-8", "complexity_score": 0.6}

            # Test multiple word definitions
            words = [("ecosystem", "6-8"), ("democracy", "9-12"), ("fraction", "3-5"), ("photosynthesis", "6-8"), ("algorithm", "College")]

            for word, grade_level in words:
                result, response_time = await performance_tester.measure_operation("get_word_definition", get_word_definition, ctx, word=word, grade_level=grade_level)
    ...
    async def test_research_paper_response_times(self, performance_tester):
        ...
        with patch("src.api.arxiv.ArxivAPI.search_papers") as mock_search:
            mock_search.return_value = [{"title": "Test Paper", "abstract": "Test abstract", "academic_level": "Graduate", "educational_relevance": 0.8}]
            ...
            for query, subject, academic_level in search_queries:
                result, response_time = await performance_tester.measure_operation("search_academic_papers", search_academic_papers, ctx, query=query, subject=subject, academic_level=academic_level)
...
class TestCachingPerformance:
    ...
    async def test_cache_hit_performance(self, performance_tester):
        ...
        with patch("src.api.openlibrary.OpenLibraryAPI.search_books") as mock_search:
            mock_search.return_value = [{"title": "Cached Book", "author": "Cache Author", "grade_level": "6-8"}]
            ...
            result1, time1 = await performance_tester.measure_operation("search_books_cache_miss", search_educational_books, ctx, query=query, subject=subject, grade_level=grade_level)

            # Second request (should be cache hit)
            result2, time2 = await performance_tester.measure_operation("search_books_cache_hit", search_educational_books, ctx, query=query, subject=subject, grade_level=grade_level)
    ...
    async def test_cache_hit_rate_target(self, performance_tester):
        ...
        with patch("src.api.wikipedia.WikipediaAPI.search_articles") as mock_search:
            mock_search.return_value = [{"title": "Test Article", "summary": "Test summary"}]
            ...
            for query in queries:
                await performance_tester.measure_operation("article_search_cache_test", search_educational_articles, ctx, query=query, grade_level="6-8")
...
class TestConcurrentRequestHandling:
    ...
    async def test_concurrent_book_searches(self, performance_tester):
        ...
        with patch("src.api.openlibrary.OpenLibraryAPI.search_books") as mock_search:
            mock_search.return_value = [{"title": "Concurrent Book", "author": "Concurrent Author"}]
            ...
            results = await performance_tester.run_concurrent_operations(
                ...
                lambda ctx, query, subject, grade_level: search_educational_books(ctx, query=query, subject=subject, grade_level=grade_level),
    ...
    async def test_grade_level_filtering_performance(self, performance_tester):
        ...
        with patch("src.api.openlibrary.OpenLibraryAPI.search_books") as mock_search:
            ...
            def mock_grade_response(*args, **kwargs):
                ...
                return [{"title": f"Book for {grade_level}", "grade_level": grade_level, "educational_value": 0.9, "complexity_score": 0.3 if grade_level == "K-2" else 0.8}]
            ...
            for grade_level in grade_levels:
                result, response_time = await performance_tester.measure_operation(f"grade_filtering_{grade_level}", search_educational_books, ctx, query="mathematics", grade_level=grade_level)
    ...
    async def test_subject_classification_performance(self, performance_tester):
        ...
        with patch("src.api.wikipedia.WikipediaAPI.search_articles") as mock_search:
            ...
            def mock_subject_response(*args, **kwargs):
                ...
                return [{"title": f"Article about {subject}", "subject": subject, "educational_value": 0.85, "grade_level": "6-8"}]
            ...
            for subject in subjects:
                result, response_time = await performance_tester.measure_operation(
                    f"subject_classification_{subject.replace(' ', '_')}", search_educational_articles, ctx, query="test", subject=subject, grade_level="6-8"
                )
    ...
    async def test_complexity_scoring_performance(self, performance_tester):
        ...
        with patch("src.api.dictionary.DictionaryAPI.get_definition") as mock_definition:
            ...
            def mock_complexity_response(*args, **kwargs):
                ...
                complexity_scores = {"K-2": 0.2, "3-5": 0.4, "6-8": 0.6, "9-12": 0.8, "College": 0.9}

                return {"word": word, "definition": f"Definition for {word}", "grade_level": grade_level, "complexity_score": complexity_scores.get(grade_level, 0.6)}

            mock_definition.side_effect = mock_complexity_response

            # Test complexity scoring for different words and grade levels
            test_cases = [("cat", "K-2"), ("ecosystem", "3-5"), ("photosynthesis", "6-8"), ("democracy", "9-12"), ("quantum", "College")]

            for word, grade_level in test_cases:
                result, response_time = await performance_tester.measure_operation(f"complexity_scoring_{word}_{grade_level}", get_word_definition, ctx, word=word, grade_level=grade_level)
...
class TestServerStatusPerformance:
    ...
    async def test_server_status_response_time(self, performance_tester):
        ...
        for i in range(5):
            result, response_time = await performance_tester.measure_operation("server_status", get_server_status, ctx)
...
async def test_performance_benchmarks(performance_tester):
    ...
    with (
        ...
        for op_name, op_func in operations:
            ...
            times = []
            for _ in range(10):
                _, response_time = await performance_tester.measure_operation(f"benchmark_{op_name.replace(' ', '_').lower()}", op_func)

...
class Article(BaseModel):
    ...
    def from_wikipedia(cls, wp_data: Dict[str, Any]) -> "Article":
        ...
        if "categories" in wp_data:
            categories_text = " ".join([cat.replace("Category:", "").lower() if isinstance(cat, str) else cat.get("title", "").replace("Category:", "").lower() for cat in wp_data["categories"]])

            educational_category_keywords = ["education", "academic", "science", "mathematics", "history", "learning", "research", "university", "school"]

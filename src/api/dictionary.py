...
class DictionaryClient:
    ...
    async def _make_request(self, url: str, params: Optional[Dict[str, Any]] = None, retry_count: int = 0) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            for phonetic in phonetics:
                if phonetic.get("text") and phonetic.get("audio"):
                    return {"text": phonetic["text"], "audio": phonetic["audio"], "source": phonetic.get("sourceUrl", "")}
                elif phonetic.get("text") and not best_phonetic.get("text"):
                    best_phonetic = {"text": phonetic["text"], "audio": phonetic.get("audio", ""), "source": phonetic.get("sourceUrl", "")}
                elif phonetic.get("audio") and not best_phonetic.get("audio"):
                    best_phonetic.update({"audio": phonetic["audio"], "source": phonetic.get("sourceUrl", "")})
    ...
    async def health_check(self) -> Dict[str, Any]:
        ...
        try:
            ...
            return {"status": "healthy" if result else "degraded", "response_time_ms": response_time, "api_accessible": bool(result), "timestamp": datetime.now().isoformat()}

        except Exception as e:
            logger.error(f"Dictionary API health check failed: {e}")
            return {"status": "unhealthy", "error": str(e), "api_accessible": False, "timestamp": datetime.now().isoformat()}

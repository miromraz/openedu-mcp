...
class RateLimitingService:
    ...
    def _initialize_rate_limits(self) -> None:
        ...
        api_configs = {"open_library": self.apis_config.open_library, "wikipedia": self.apis_config.wikipedia, "dictionary": self.apis_config.dictionary, "arxiv": self.apis_config.arxiv}
    ...
    async def check_rate_limit(self, api_name: str) -> bool:
        ...
        async with self._locks[api_name]:
            ...
            if state.current_count >= config.rate_limit:
                ...
                raise RateLimitError(f"Rate limit exceeded for {api_name}", api_name, retry_after=max(retry_after, 1))
    ...
    def _get_api_config(self, api_name: str):
        ...
        config_map = {"open_library": self.apis_config.open_library, "wikipedia": self.apis_config.wikipedia, "dictionary": self.apis_config.dictionary, "arxiv": self.apis_config.arxiv}
        ...
        for api_name, state in self.rate_limits.items():
            async with self._locks[api_name]:
                ...
                api_stats[api_name] = {
                    "total_requests": state.total_requests,
                    "blocked_requests": state.blocked_requests,
                    "success_rate": ((state.total_requests - state.blocked_requests) / max(state.total_requests, 1)) * 100,
                }

        return {
            "total_requests": total_requests,
            "total_blocked": total_blocked,
            "overall_success_rate": ((total_requests - total_blocked) / max(total_requests, 1)) * 100,

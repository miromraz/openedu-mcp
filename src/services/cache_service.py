...
class CacheService:
    ...
    async def set(self, key: str, value: Any, ttl: Optional[int] = None, content_type: str = "json") -> None:
        """
        ...
        try:
            ...
            async with aiosqlite.connect(self.db_path) as db:
                await db.execute(
                    ...
                    VALUES (?, ?, ?, ?, ?, ?, ?)
                """,
                    (key, value_blob, content_type, expires_at, size_bytes, datetime.now(), datetime.now()),
                )
    ...
    async def cleanup_expired(self) -> int:
        ...
        try:
            async with aiosqlite.connect(self.db_path) as db:
                cursor = await db.execute("DELETE FROM cache_entries WHERE expires_at <= ?", (datetime.now(),))
                await db.commit()
    ...
    async def get_stats(self) -> Dict[str, Any]:
        ...
        try:
            async with aiosqlite.connect(self.db_path) as db:
                ...
                # Expired entries
                cursor = await db.execute("SELECT COUNT(*) FROM cache_entries WHERE expires_at <= ?", (datetime.now(),))

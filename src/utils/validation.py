...
class Validator:
    ...
    def validate_query(query: str, min_length: int = 1, max_length: int = 500) -> str:
        ...
        if len(query) < min_length:
            raise ValidationError(f"Query must be at least {min_length} characters long", "query")

        if len(query) > max_length:
            raise ValidationError(f"Query must be no more than {max_length} characters long", "query")

        # Check for potentially harmful content
        if re.search(r"[<>]", query):
            raise ValidationError("Query contains invalid characters", "query")
    ...
    def validate_grade_level(grade_level: str) -> GradeLevel:
        ...
        raise ValidationError(f"Invalid grade level. Must be one of: {', '.join(valid_levels)}", "grade_level")
    ...
    def validate_language_code(language: str) -> str:
        ...
        valid_languages = {"en", "es", "fr", "de", "it", "pt", "ru", "zh", "ja", "ko", "ar", "hi", "nl", "sv", "no", "da", "fi", "pl", "tr", "he"}

        if language not in valid_languages:
            raise ValidationError(f"Unsupported language code: {language}", "language")

        return language

    @staticmethod
    def validate_date_range(start_date: Optional[Union[str, date]], end_date: Optional[Union[str, date]]) -> tuple[Optional[date], Optional[date]]:
        """
        Validate date range.

...
class WikipediaTool(BaseTool):
    ...
    def __init__(self, config: Config, cache_service: CacheService, rate_limiting_service: RateLimitingService, usage_service: UsageService):
        ...
        self.reading_level_thresholds = {GradeLevel.K_2: 500, GradeLevel.GRADES_3_5: 1000, GradeLevel.GRADES_6_8: 2000, GradeLevel.GRADES_9_12: 4000, GradeLevel.COLLEGE: 8000}
    ...
    async def search_educational_articles(
        self, query: str, subject: Optional[str] = None, grade_level: Optional[str] = None, language: str = "en", limit: int = 10, user_session: Optional[str] = None
    ) -> List[Dict[str, Any]]:
        """
        ...
        async def _search():
            # Validate parameters
            validated = await self.validate_common_parameters(query=query, limit=limit, grade_level=grade_level, subject=subject, language=language)
            ...
            raw_articles = await self.client.search_wikipedia(search_query, validated.get("language", "en"), validated["limit"])
            ...
            for article_data in raw_articles:
                try:
                    ...
                    article = await self._enrich_educational_metadata(article, subject, grade_level, language)
            ...
            filtered_articles = self._apply_educational_filters(articles, grade_level=validated.get("grade_level"), subject=subject)
        ...
        return await self.execute_with_monitoring(
            "search_educational_articles", _search, user_session=user_session, cache_params={"query": query, "subject": subject, "grade_level": grade_level, "language": language, "limit": limit}
        )

    async def get_article_summary(self, title: str, language: str = "en", include_educational_analysis: bool = True, user_session: Optional[str] = None) -> Dict[str, Any]:
        """
        ...
        return await self.execute_with_monitoring("get_article_summary", _get_summary, user_session=user_session, cache_params={"title": title, "language": language})

    async def get_article_content(self, title: str, language: str = "en", include_images: bool = False, user_session: Optional[str] = None) -> Dict[str, Any]:
        """
        ...
        return await self.execute_with_monitoring("get_article_content", _get_content, user_session=user_session, cache_params={"title": title, "language": language, "include_images": include_images})

    async def get_featured_article(self, date_param: Optional[str] = None, language: str = "en", user_session: Optional[str] = None) -> Dict[str, Any]:
        """
        ...
        return await self.execute_with_monitoring("get_featured_article", _get_featured, user_session=user_session, cache_params={"date_param": date_param, "language": language})

    async def get_articles_by_subject(self, subject: str, grade_level: Optional[str] = None, language: str = "en", limit: int = 10, user_session: Optional[str] = None) -> List[Dict[str, Any]]:
        """
        ...
        async def _search_by_subject():
            # Validate parameters
            validated = await self.validate_common_parameters(subject=subject, limit=limit, grade_level=grade_level, language=language)
            ...
            for article_data in raw_articles:
                try:
                    article = Article.from_wikipedia(article_data)
                    article = await self._enrich_educational_metadata(article, subject=subject, grade_level=grade_level, language=language)
                    articles.append(article)
                except Exception as e:
                    logger.warning(f"Failed to process article data: {e}")
                    continue
        ...
        return await self.execute_with_monitoring(
            "get_articles_by_subject", _search_by_subject, user_session=user_session, cache_params={"subject": subject, "grade_level": grade_level, "language": language, "limit": limit}
        )

    async def _enrich_educational_metadata(self, article: Article, subject: Optional[str] = None, grade_level: Optional[str] = None, language: str = "en") -> Article:
        """
    ...
    def _calculate_educational_relevance(self, article: Article, target_subject: Optional[str] = None, target_grade_level: Optional[str] = None) -> float:
        ...
        if article.categories:
            educational_categories = ["education", "science", "mathematics", "history", "literature", "academic", "research", "learning", "teaching"]
    ...
    def _analyze_reading_level(self, article: Article) -> Dict[str, str]:
        ...
        return {"level": level, "difficulty": difficulty, "word_count": word_count, "complexity_score": complex_indicators}
    ...
    def _enhance_subject_classification(self, categories: List[str], target_subject: Optional[str] = None) -> List[str]:
    ...
    def _analyze_curriculum_alignment(self, article: Article, subject: Optional[str] = None) -> List[str]:
        ...
        ngss_indicators = ["engineering design", "scientific practices", "crosscutting concepts", "disciplinary core ideas", "phenomena", "systems"]
    ...
    def _extract_educational_topics(self, article: Article) -> List[str]:
        ...
        if article.categories:
            ...
            educational_cats = [
                ...
                if any(keyword in cat.lower() for keyword in ["education", "science", "mathematics", "history", "literature", "learning", "academic", "research", "study"])
            ]
        ...
        if article.content:
            ...
            educational_terms = [term for term in content_words if len(term) > 3 and len(term.split()) <= 3]
    ...
    def _apply_educational_filters(
        self, articles: List[Article], grade_level: Optional[GradeLevel] = None, subject: Optional[str] = None, min_relevance_score: Optional[float] = None
    ) -> List[Article]:
        ...
        filtered = [article for article in filtered if article.get_educational_score() >= min_score]

        # Filter by grade level appropriateness
        if grade_level and self.enable_age_appropriate:
            filtered = [article for article in filtered if not article.educational_metadata.grade_levels or grade_level in article.educational_metadata.grade_levels]
        ...
        if subject:
            filtered = [
                ...
                if any(subject.lower() in subj.lower() for subj in article.educational_metadata.educational_subjects)
    ...
    async def health_check(self) -> Dict[str, Any]:
        ...
        try:
            ...
            return {"tool_name": self.tool_name, "api_name": self.api_name, "status": "unhealthy", "error": str(e), "timestamp": datetime.now().isoformat()}

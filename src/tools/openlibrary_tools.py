...
class OpenLibraryTool(BaseTool):
    ...
    async def search_educational_books(self, query: str, subject: Optional[str] = None, grade_level: Optional[str] = None, limit: int = 10, user_session: Optional[str] = None) -> List[Dict[str, Any]]:
        """
        ...
        async def _search():
            # Validate parameters
            validated = await self.validate_common_parameters(query=query, limit=limit, grade_level=grade_level, subject=subject)
            ...
            filtered_books = self._apply_educational_filters(books, grade_level=validated.get("grade_level"), subject=subject)
        ...
        return await self.execute_with_monitoring(
            "search_educational_books", _search, user_session=user_session, cache_params={"query": query, "subject": subject, "grade_level": grade_level, "limit": limit}
        )

    async def get_book_details_by_isbn(self, isbn: str, include_cover: bool = True, user_session: Optional[str] = None) -> Dict[str, Any]:
        """
        ...
        return await self.execute_with_monitoring("get_book_details_by_isbn", _get_details, user_session=user_session, cache_params={"isbn": isbn})

    async def search_books_by_subject(self, subject: str, grade_level: Optional[str] = None, limit: int = 10, user_session: Optional[str] = None) -> List[Dict[str, Any]]:
        """
        ...
        async def _search_by_subject():
            # Validate parameters
            validated = await self.validate_common_parameters(subject=subject, limit=limit, grade_level=grade_level)

            # Search by subject
            raw_books = await self.client.search_by_subject(validated["subject"], validated["limit"])
            ...
            for book_data in raw_books:
                try:
                    book = Book.from_open_library(book_data)
                    book = await self._enrich_educational_metadata(book, subject=subject, grade_level=grade_level)
                    books.append(book)
                except Exception as e:
                    logger.warning(f"Failed to process book data: {e}")
                    continue
        ...
        return await self.execute_with_monitoring(
            "search_books_by_subject", _search_by_subject, user_session=user_session, cache_params={"subject": subject, "grade_level": grade_level, "limit": limit}
        )

    async def get_book_recommendations(self, grade_level: str, subject: Optional[str] = None, limit: int = 10, user_session: Optional[str] = None) -> List[Dict[str, Any]]:
        """
        ...
        async def _get_recommendations():
            # Validate parameters
            validated = await self.validate_common_parameters(grade_level=grade_level, subject=subject, limit=limit)
        ...
        return await self.execute_with_monitoring(
            "get_book_recommendations", _get_recommendations, user_session=user_session, cache_params={"grade_level": grade_level, "subject": subject, "limit": limit}
        )

    async def _enrich_educational_metadata(self, book: Book, subject: Optional[str] = None, grade_level: Optional[str] = None) -> Book:
        """
    ...
    def _calculate_educational_relevance(self, book: Book, target_subject: Optional[str] = None, target_grade_level: Optional[str] = None) -> float:
        ...
        # Subject relevance
        if target_subject:
            subject_match = any(target_subject.lower() in subject.lower() for subject in (book.subjects + book.educational_metadata.educational_subjects))
            if subject_match:
                score += 0.4
        ...
        educational_keywords = ["education", "learning", "teaching", "student", "curriculum", "textbook", "workbook", "study", "academic", "school"]
    ...
    def _apply_educational_filters(self, books: List[Book], grade_level: Optional[GradeLevel] = None, subject: Optional[str] = None, min_relevance_score: Optional[float] = None) -> List[Book]:
        ...
        filtered = self.filter_by_educational_criteria(filtered, grade_level=grade_level.value if grade_level else None, subject=subject, min_relevance_score=min_relevance_score)
    ...
    async def health_check(self) -> Dict[str, Any]:
        ...
        try:
            ...
            return {"status": "healthy", "api_health": api_health, "test_search_results": len(test_books), "timestamp": datetime.now().isoformat(), "tool_name": self.tool_name}

        except Exception as e:
            return {"status": "unhealthy", "error": str(e), "timestamp": datetime.now().isoformat(), "tool_name": self.tool_name}
